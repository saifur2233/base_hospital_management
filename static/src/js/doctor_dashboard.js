...
export class DoctorDashboard extends Component {
    ...
    async _loadSummaryCounts() {
        const [
            ...
            this.orm.call('res.partner', 'search_count', [[['patient_seq', 'not in', ['New', 'Employee', 'User']]]]),
            this.orm.call('hospital.inpatient', 'search_count', [[]]),
            this.orm.call('inpatient.surgery', 'search_count', [[]]),
            this.orm.call('doctor.allocation', 'search_count', [[]]),
        ]);

        this.state.summary.consultation = consultation || 0;
        this.state.summary.patient = patient || 0;
        this.state.summary.inpatient = inpatient || 0;
        this.state.summary.surgery = surgery || 0;
        this.state.summary.allocation = allocation || 0;
    }
